...
TEST_F(LoadAccumulatorWithMemory, AbsoluteIndexedWithY)
{
    ...
    const uint8_t     y_register    = 32;

    loadInstructionIntoMemory(AbstractInstruction_e::LDA, AddressMode_e::AbsoluteYIndexed, address);
    fakeMemory[address + 1] = loByteOf(address_to_load_from);
    fakeMemory[address + 2] = hiByteOf(address_to_load_from);
    fakeMemory[address_to_load_from + y_register] = value_to_load;
    ...
    EXPECT_THAT(fakeMemory.at(address_to_load_from + y_register), Eq(value_to_load));
